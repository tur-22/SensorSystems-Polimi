...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint8_t rx_buffer[64]; // string to contain what is received through USART
uint8_t lcd_buffer[64]; // string to contain what will be displayed on the LCD
//int i = 0;
/* USER CODE END 0 */
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size) {
	if (huart->Instance == USART2) {
		...
		memcpy(&lcd_buffer, &rx_buffer, Size); // copy only what was received
...
/*
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) { // attempt at reading one byte at a time
	if (rx_data != '\%') {
		to_print[i] = rx_data;
		i++;
	}
	lcd_println(to_print, 1);
	if (rx_data == '\%') {
